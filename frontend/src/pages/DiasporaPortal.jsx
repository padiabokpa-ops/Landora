...
export default function DiasporaPortal() {
  ...
  return (
    <div style={{ background: 'var(--cream)' }}>
      {/* HERO */}
      <section style={{ background: 'linear-gradient(135deg, #C9933B 0%, #E8B65A 100%)', color: 'white', padding: '60px 0' }}>
        <div className="container" style={{ padding: '0 16px' }}>
          <div className="diaspora-hero">
            ...
            <div>
              ...
              <h1 style={{ fontFamily: 'var(--ff-display)', fontSize: 'clamp(28px, 4vw, 52px)', fontWeight: 700, margin: '16px 0 20px', lineHeight: 1.15 }}>
                Own a Piece of Nigeria<br />From Anywhere on Earth
              </h1>
              <p style={{ fontSize: 16, opacity: 0.9, lineHeight: 1.7, marginBottom: 32 }}>
            ...
            <div style={{ background: 'white', borderRadius: 20, padding: 32, boxShadow: '0 20px 60px rgba(0,0,0,0.15)', color: 'var(--text)' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                <input
                  ...
                  style={{ flex: 1, minWidth: 0, padding: '12px 14px', border: '1.5px solid var(--border)', borderRadius: 8, fontSize: 16, outline: 'none' }}
                />
                <select value={calcCurrency} onChange={e => setCalcCurrency(e.target.value)}
                  style={{ padding: '12px 10px', border: '1.5px solid var(--border)', borderRadius: 8, fontSize: 14, background: 'white', outline: 'none' }}>
      ...
      <section style={{ background: 'white', padding: '32px 0', borderBottom: '1px solid var(--border)' }}>
        <div className="container" style={{ padding: '0 16px', display: 'flex', justifyContent: 'center', gap: 32, flexWrap: 'wrap' }}>
          {['🔒 Escrow Protected', '📄 Title Verified', '📹 Virtual Tours', '🌍 Multi-currency', '⚖️ Legal Documentation'].map(b => (
            <span key={b} style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)' }}>{b}</span>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '60px 0 80px', background: 'white' }}>
        <div className="container" style={{ maxWidth: 780, padding: '0 16px' }}>
          ...
          {FAQ.map((f, i) => (
            <div key={i} style={{ borderBottom: '1px solid var(--border)', overflow: 'hidden' }}>
              <button onClick={() => setOpenFaq(openFaq === i ? null : i)} style={{
                ...
                fontSize: 15, fontWeight: 600, color: 'var(--dark)', cursor: 'pointer', gap: 12
              }}>
                <span>{f.q}</span>
                <span style={{ fontSize: 20, color: 'var(--green)', flexShrink: 0, transition: 'transform 0.2s', transform: openFaq === i ? 'rotate(45deg)' : 'none' }}>+</span>
              </button>
              {openFaq === i && (
